...
public class ShopAnalyzer extends BasicGuiScreen implements EventListener {
	private HashSet<Shop> shopSet = new HashSet();
	...
	private Item boundItem = Items.book;
		...
		if(!possibleModes.contains(modeField.getText().toLowerCase())) {
			...
			if(modeField.getText().equalsIgnoreCase("name")) {
				...
				for(String s : itemNameResultSet) {
					drawString(fontRendererObj, s, width / 2 - 65 + (i % 3) * 85, inputGuiTop - 12 + (i / 3) * 12, 16777215);
					++i;
				}
			} else {
				for(int i = 0; i < viewResult.size(); ++i) {
					drawString(fontRendererObj, viewResult.get(i), width / 2 - 60, inputGuiTop - 12 + i * 12, 16777215);
				}
				int i = 0;
				for(String s : itemNameResultSet) {
					if(i == autoCompleteIndex) {
						drawString(fontRendererObj, "=>", width / 2 - 175, inputGuiTop + 50 + i * 12, 16777215);
					}
					drawString(fontRendererObj, s, width / 2 - 160, inputGuiTop + 50 + i * 12, 16777215);
					++i;
				}
			}
		}

		drawString(fontRendererObj, "Time needed: " + getTimeNeeded(), width / 2 - 160, 8, 16777215);
		drawString(fontRendererObj, "Total shop count: " + shopSet.size(), width / 2, 8, 16777215);
		
		drawString(fontRendererObj, "Item Name", width / 2 - 160, inputGuiTop - 12, 16777215);
		itemNameField.drawTextBox();
		
		drawString(fontRendererObj, "Mode", width / 2 - 160, inputGuiTop + 18, 16777215);
	...
	public void onCommand(CommandEvent event) {
		if(event.getCommand().equalsIgnoreCase("shop")) {
			...
			if(event.getArg(1).equalsIgnoreCase("name")) {
				...
				sender.sendSilent("Number of shop's in total: " + shopSet.size());
			...
			} else if(event.getArg(1).equalsIgnoreCase("clear")) {
				sender.sendSilent("The list of all shops was cleared.");
				shopSet.clear();
